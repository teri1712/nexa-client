...
export class AuthService extends TokenStore implements IAuthService {
      ...
      loginWithCredentials(credentials: AdminLoginRequest) {
            return this.http
                    .post<AccountResponse>(`${this.base}/login`, credentials)
                    .pipe(tap(res =>
                                    this.storeSession(res.profile, res.accessToken)),
                            map(account => account.profile));
      }
      ...
      changePassword(newPassword: string, currentPassword: string) {
            ...
            if (currentPassword) {
                  body = body.set('password', currentPassword);
            }

            return this.http.post<ProfileResponse>(
                    `${this.base}/profiles/me/password`,
                    body,
                    {
                    ...
                    switchMap(profile => this.relogin(profile.username, newPassword))
            );
      }
}
